 ...
 * @author julio
 */
public class consulta_compracredito extends javax.swing.JFrame {
   ...
   double total=0;
    ...
    void cargartodascompras()
    {
        ...
        String []titulos={"NUMERO","COD. CLIENTE","NOMBRE CLIENTE","TOTAL","FECHA"};
        tabla.setColumnIdentifiers(titulos);
        this.jTbuscar.setModel(tabla);
        String consulta= "SELECT * FROM compracredito";
        ...
        try {
            ...
            while(rs.next())
            {
                ...
                Datos[1]=rs.getString("cod_prov");
                Datos[2]=rs.getString("nom_prov");
            ...
            Logger.getLogger(consulta_compracredito.class.getName()).log(Level.SEVERE, null, ex);
        }
    ...
    private void initComponents() {
        ...
        jLabel1.setText("Consulta Compras al Credito");
        jPanel1.add(jLabel1);
        jLabel1.setBounds(150, 20, 310, 29);
    ...
    {
        consulta= "SELECT * FROM compracredito WHERE num_comp='"+num+"'";
    }
    
    if(rdbntodos.isSelected()==true)
    {
        consulta= "SELECT * FROM compracredito ";
    }
        DefaultTableModel tabla= new DefaultTableModel();
        String []titulos={"NUMERO","COD. CLIENTE","NOMBRE CLIENTE","TOTAL","FECHA"};
        tabla.setColumnIdentifiers(titulos);
    ...
    {
           detalle_compracredito detalle1 = new detalle_compracredito();
    detalle1.toFront();
    detalle1.setVisible(true);
        String numbol=jTbuscar.getValueAt(filasele, 0).toString();
        String cod=jTbuscar.getValueAt(filasele, 1).toString();
        String nomclie=jTbuscar.getValueAt(filasele, 2).toString();
        String total=jTbuscar.getValueAt(filasele, 3).toString();
        String fecha=jTbuscar.getValueAt(filasele, 4).toString();
        
        detalle_compracredito.txtfac.setText(numbol);
        detalle_compracredito.txtcod.setText(cod);
        detalle_compracredito.txtnom.setText(nomclie);
        detalle_compracredito.txttot.setText(total);
        detalle_compracredito.txtfecha.setText(fecha);
        
        DefaultTableModel model = (DefaultTableModel) detalle_compracredito.tbdetalle.getModel();
        String ver="SELECT * FROM detallecompracredito WHERE num_comp='"+numbol+"'";
        String []datos= new String[5]   ;
        try {
                Statement st = cn.createStatement();
                ...
                detalle_compracredito.tbdetalle.setModel(model);
            } catch (SQLException ex) {
                Logger.getLogger(consulta_compracredito.class.getName()).log(Level.SEVERE, null, ex);
            }
            }
    ...
    if(fila>=0)
    {
        String cod=jTbuscar.getValueAt(fila, 0).toString();
            try {
                PreparedStatement pst = cn.prepareStatement("DELETE FROM compracredito WHERE num_comp='"+cod+"'");
                pst.executeUpdate();
            } catch (SQLException ex) {
                Logger.getLogger(consulta_compracredito.class.getName()).log(Level.SEVERE, null, ex);
            }
